...
impl ProcessSnapshot {
    ...
    fn next(&mut self) -> Option<Result<ProcessEntry, windows::core::Error>> {
        if self.is_empty {
            return None;
        }
        let proc_entry_ptr = std::ptr::addr_of_mut!(self.winapi_module_entry);
        if self.is_first {
            self.is_first = false;
            // SAFETY: this call is safe, because
        ...
        let mut process_name =
            String::from_utf16(&self.winapi_module_entry.szExeFile).unwrap_or_default();
...
impl ModuleSnapshot {
    ...
    fn next(&mut self) -> Option<Result<ModuleEntry, windows::core::Error>> {
        if self.is_empty {
            return None;
        }
...
impl ModuleEntry {
    /// Returns a mutable view of module memory
    ///
    /// SAFETY:
    ...
    /// 4) there must be no other references to the underlying memory as long as this view lives
